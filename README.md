...
This directory only estimates biomass density in the Amazon basin, allowing the rest of the Use Case to be carried on. It uses a slightly modified version of the pretrained [Copernicus-FM v1](https://github.com/zhu-xlab/Copernicus-FM) multimodal foundation model, with additional ML components, to obtain 1-channel grayscales of 264 x 264 patches (real life size: 2,64 x 2,64 km), predicting the biomass density of a 100 m x 100 m resolution Amazon basin patch (extracted from ESA data). Its ability to process a wide variety of spectral data is especially useful for biomass density estimation in forests.
